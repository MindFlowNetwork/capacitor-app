...
const Camera = registerPlugin("Camera", {
  web: () => __vitePreload(() => import("./web.8b9558b1.js"), true ? [] : void 0).then((m) => new m.CameraWeb())
});
...
const q = [], Wt = Promise.resolve(), V = (e) => Wt.then(e), Ie = (e) => {
  ...
  if (e.tagName === "SELECT") {
    ...
    T(e, "change", () => {
      ...
      for (let p2 = 0, x = f.options.length; p2 < x; p2++) {
        const b = f.options[p2], v = A(b);
        if (u)
          y(a) ? b.selected = G(a, v) > -1 : b.selected = a.has(v);
        else if (I(A(b), a)) {
          f.selectedIndex !== p2 && (f.selectedIndex = p2);
          return;
        }
      }
  ...
  return n.effect(() => {
    for (let p2 = 0; p2 < i.length; p2++) {
      const { exp: x, el: b } = i[p2];
      if (!x || H(n.scope, x)) {
        p2 !== a && (u(), f = new ue(b, n), f.insert(s, r), s.removeChild(r), a = p2);
        return;
      }
    }
    a = -1, u();
  ...
  const o = s[2].trim();
  let l = s[1].trim().replace(ln, "").trim(), f, a = false, u, p2, x = "key", b = e.getAttribute(x) || e.getAttribute(x = ":key") || e.getAttribute(x = "v-bind:key");
  b && (e.removeAttribute(x), x === "key" && (b = JSON.stringify(b)));
  let v;
  (v = l.match(Le)) && (l = l.replace(Le, "").trim(), u = v[1].trim(), v[2] && (p2 = v[2].trim())), (v = l.match(fn)) && (f = v[1].split(",").map((m) => m.trim()), a = l[0] === "[");
  let pe = false, R, L, J;
  const et = (m) => {
    ...
    const g = {};
    f ? f.forEach((M, E) => g[M] = w[a ? E : M]) : g[l] = w, d ? (u && (g[u] = d), p2 && (g[p2] = h)) : u && (g[u] = h);
